...
public:
  ...
  // and by doc_id
  void sortWordRelativeIndex(const int &indexRequest, const float &rank,
                             const size_t &doc_id,
                             std::vector<std::vector<RelativeIndex>> &search);

  // Method of adding by word from query to relevant responses
  void addWordRelevantIndex(const int &indexRequest, const std::string &word,
                            int &maxRelevant,
                            std::vector<std::vector<RelativeIndex>> &search);
  ...
  search(const std::vector<std::string> &queries_input, int max_responses);
